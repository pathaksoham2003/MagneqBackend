...
const mockData = JSON.parse(fs.readFileSync(new URL('./rawMaterials.json', import.meta.url), 'utf-8'));
...
const seedUsers = async () => {
  try {
    const users = JSON.parse(fs.readFileSync(new URL('./users.json', import.meta.url), 'utf-8'));
...
const generateFinishedGoods = async (rawMaterials) => {
  ...
  if (!classA || !classB || !classC) {
    console.error('❌ Could not find required raw materials: class A, processed class B, and class C');
    return;
  }
